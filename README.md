...
不采信:0x98 附近(cclk_setpoint 与 socket_power 二义)、0x540+ 的 L3/GFX 温度(单位异常)。

### 每核数组(逐核钉载差分破译,已接入)

逐核钉载扫描(10 物理核 × 13 状态)+ 基准差分,破译出三个每核数组,槽位对角线完美(钉哪个核哪个槽跳):

| 基址 | 字段 | 单位 | 实测(钉核/深睡) |
|---|---|---|---|
| 0x9DC + 4×槽 | 每核功率 | W | 11.7W / 0.1-0.6W |
| 0xC1C + 4×槽 | 每核时钟 | **0.25 MHz** | 4464MHz / 11-120MHz |
| 0xC7C + 4×槽 | 每核 VID | V | ~0.99V / 0.05-0.11V |

槽位映射(硬件核号 → 表槽):{0→0, 1→1, 2→2, 3→3, 8→4, 9→6, 10→7, 11→9, 12→10, 13→11}——12 槽中 10 个占用,槽 5/8 是硅片禁用空位。满载校验:Σ每核功率 ≈ 50W 与 RAPL Package 58.9W(含平台)自洽,全核时钟 3.3-3.76GHz 与 94°C 热钳状态吻合。
